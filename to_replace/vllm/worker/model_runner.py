    ...
    def execute_model(
        ...
        **kwargs,
        ...
        if os.getenv("remaining", "remaing") == "remaing":
            ...
            A = [7, 12548, 7274, 2878, 220,]
            B = [11211,476,16745,11,220,22,3281,11211,320,220,151671]
            
            budget, a_indices, b_indices = None, None, None
            for seq_id in range(len(model_input.sampling_metadata.seq_groups)):
                prompt_token_ids = next(iter(model_input.sampling_metadata.seq_groups[seq_id].seq_data.values())).prompt_token_ids
                
                prompt_token_ids_list = list(prompt_token_ids)
                for i in range(len(prompt_token_ids_list)):
                    A_len = len(A)
                    B_len = len(B)
                    if prompt_token_ids_list[i:i+A_len] == A:
                        a_indices = i+A_len
                    if prompt_token_ids_list[i:i+B_len] == B:
                        b_indices = i

                if a_indices is not None and b_indices is not None:
                    between_a_and_b = prompt_token_ids_list[a_indices:b_indices]
                    try:
                        delta_ids = 15
                        budget = 0
                        length = len(between_a_and_b)
                        for i in range(length):
                            budget += (between_a_and_b[i] - delta_ids) * (10 ** (length - i - 1))
                        break  # 找到第一个匹配的就退出循环
                    except ValueError:
                        continue
            ...
            if budget is None:
                ...
                print(f"prompt_token_ids = {prompt_token_ids_list}")
        ...
        # For example, a seq_groups[i] is like: SequenceGroupToSample(seq_ids=[2], sampling_params=SamplingParams(n=1, presence_penalty=0.0, frequency_penalty=0.0, repetition_penalty=1.0, temperature=0.5, top_p=1, top_k=-1, min_p=0.0, seed=None, stop=['</s>', '<|im_end|>', '<|endoftext|>', '<｜end▁of▁sentence｜>'], stop_token_ids=[], bad_words=[], include_stop_str_in_output=False, ignore_eos=False, max_tokens=1500, min_tokens=0, logprobs=None, prompt_logprobs=None, skip_special_tokens=True, spaces_between_special_tokens=True, truncate_prompt_tokens=None, guided_decoding=None), seq_data={2: SequenceData(prompt_token_ids=array('l', [151646, 27, 91, 1474, 91, 29, 8948, 198, 5598, 697, 1590, 2033, 2878, 1124, 79075, 46391, 7169, 279, 1459, 4930, 15, 11, 18, 15087, 304, 51424, 13934, 311, 24660, 13934, 13, 220, 11252, 697, 4226, 304, 279, 1352, 4930, 81, 26266, 15976, 98406, 1380, 400, 81, 861, 220, 15, 3, 323, 400, 15, 1124, 273, 1124, 15976, 366, 220, 17, 1124, 2493, 2418, 151684, 151645]), output_token_ids=(198, 1654, 5185, 429), cumulative_logprob=inf, get_num_computed_tokens=68)}, seq_len=None, query_len=1, generator=None, is_prompt=False, prompt_logprob_indices=[], sample_indices=[2])
        # according to prompt token ids, find the last special token, revise output.ouptuts[i].output_token
        # an output is like: SamplerOutput(outputs=[CompletionSequenceGroupOutput(samples=[SequenceOutput(parent_seq_id=0, output_token=28301, logprobs={28301: Logprob(logprob=inf, rank=None, decoded_token=None)})], prompt_logprobs=None), CompletionSequenceGroupOutput(samples=[SequenceOutput(parent_seq_id=1, output_token=13934, logprobs={13934: Logprob(logprob=inf, rank=None, decoded_token=None)})], prompt_logprobs=None), CompletionSequenceGroupOutput(samples=[SequenceOutput(parent_seq_id=2, output_token=279, logprobs={279: Logprob(logprob=inf, rank=None, decoded_token=None)})], prompt_logprobs=None)], sampled_token_probs=None, sampled_token_ids=None, spec_decode_worker_metrics=None)
